...
Fit a datacube with the PSFAO model over all the wavelengths.

This produce is created for the 
...
3/ Change the name to psf_fitting

"""
# import library 
from typing import KeysView
from astropy.extern.configobj.validate import is_boolean
...
def plot_r0_wvl (r0: np.ndarray,x0: float, wvl: np.ndarray, index: np.ndarray, pow: float = 6./5., save = False) -> None: 
    """
...
def psf_fitting (data: np.ndarray, guess: np.ndarray, samp: int, fixed: list,ron=muse_nfm.ron, fixed_k :int = int(5), system = muse_nfm):
    """[summary]

    Args:
        data (np.ndarray): [description]
        guess (np.ndarray): [description]
        samp (int): [description]
        fixed (list): Mask for fitting parameters (True: fixed, False: free). 
        ron (int, optional): [description]. Defaults to muse_nfm.ron.
        fixed_k (int, optional): [description]. Defaults to int(5).
        system ([type], optional): [description]. Defaults to muse_nfm.

    Returns:
        fit (class): a class which contains 
            .x (np.ndarray): optimised parameters
            .dxdy (tuple): PSF shift at optimum
    ...
    # Weighting for the PSF Fitting or you can have same weight for the same pixel (e.g. np.ones_like(PSF[i,...]))
    weights = 1.0/(data*(data>0)+ron**2)

    # Masking for the bad pixels (Nan and negative pixels)
    bad = np.where(data.mask == True)  # masking the nan values in here 
    weights[bad[0],bad[1]] = 0. # for weight, reading the index 

    fit = psffit(data,Psfao,guess,weights=weights,system=muse_nfm,fixed_k = fixed_k,samp=samp,fixed=fixed)
...
def main ():
    ...
    # TODO - move this in CMD or input! 
